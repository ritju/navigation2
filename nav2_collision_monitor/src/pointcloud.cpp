...
PointCloud::PointCloud(const nav2_util::LifecycleNode::WeakPtr& node, const std::string& source_name,
  ...
  : Source(node, source_name, tf_buffer, base_frame_id, global_frame_id, transform_tolerance, source_timeout)
  , data_(nullptr)
{
...
void PointCloud::configure()
{
  ...
  data_sub_ = node->create_subscription<sensor_msgs::msg::PointCloud2>(
      source_topic, pointcloud_qos, std::bind(&PointCloud::dataCallback, this, std::placeholders::_1));
}
...
void PointCloud::getData(const rclcpp::Time& curr_time, std::vector<Point>& data) const
{
  ...
  // Get base -> global transform for ignore polygon filtering
  tf2::Transform tf_base_to_global;
  ...
  for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z)
  {
    ...
    {
      // Check ignore polygon filter in global frame
