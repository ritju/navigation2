...
Source::Source(const nav2_util::LifecycleNode::WeakPtr& node, const std::string& source_name,
...
void Source::setIgnoreManager(const std::shared_ptr<nav2_ignore_polygon_manager::IgnorePolygonManager>& ignore_manager)
{
  ignore_manager_ = ignore_manager;
}
...
void Source::getCommonParameters(std::string& source_topic)
{
  ...
  nav2_util::declare_parameter_if_not_declared(node, source_name_ + ".topic",
...
bool Source::getTransform(const std::string& source_frame_id, const rclcpp::Time& source_time,
                          const rclcpp::Time& curr_time, tf2::Transform& tf2_transform) const
{
  ...
  try
  {
    ...
    transform = tf_buffer_->lookupTransform(base_frame_id_, curr_time, source_frame_id, source_time, global_frame_id_,
                                            transform_tolerance_);
  }
  catch (tf2::TransformException& e)
  {
    RCLCPP_ERROR(logger_, "[%s]: Failed to get \"%s\"->\"%s\" frame transform: %s", source_name_.c_str(),
                 source_frame_id.c_str(), base_frame_id_.c_str(), e.what());
    return false;
  }
