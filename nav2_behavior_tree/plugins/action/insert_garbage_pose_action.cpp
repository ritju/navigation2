  ...
  const capella_ros_msg::msg::GarbageDetect::SharedPtr msg)
{
  ...
  // 1 秒内第二帧对上同一堆才往下传；对不上先放 tmp_list_，超时删掉
  if (confirm_r > 1e-6) {
...
void InsertGarbagePose::logblockSinglePileIntake()
{
