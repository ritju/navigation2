    ...
    const nav_2d_msgs::msg::Path2D &)
{
    ...
    rclcpp::Clock steady_clock_{RCL_STEADY_TIME};
    if(next_time >= predict_person_vel_time){
      // RCLCPP_INFO(rclcpp::get_logger("time"), "next time: %f", next_time);
      predict_person_vel = (sqrt(pow(pose_x - previous_goal_x, 2) + pow(pose_y - previous_goal_y, 2)))/next_time;
    ...
    predict_person_distance = sqrt(pow(robot_x - pose_x, 2) + pow(robot_y - pose_y, 2));
    // queue_dxy.push_back(predict_person_distance);
    // if(queue_dxy.size() > 2){
    //     queue_dxy.pop_front();
    // }
    // message.position.x = predict_person_vel;
    // message.position.y = predict_person_distance;
    // message.position.z = difference;
    // command_publisher_->publish(message);
    // RCLCPP_INFO(rclcpp::get_logger("distance"), "distance in goal threshold: %f, predict_person_distance: %f", predict_person_vel, predict_person_distance);
    return true;
}
...
double BaseGoalComputeVelCritic::scoreTrajectory(const dwb_msgs::msg::Trajectory2D & traj){
    ...
    if(follow_person_){
      if(predict_person_distance < min_distance_ - 0.2){
        if(traj.velocity.x < 0.01){
          score = 0;
        }
      }
      else if(predict_person_distance < min_distance_){
        if(fabs(traj.velocity.x - min_x_ + 0.1) < 0.03){
          score = 0;
        }
      }
      // 机器人速度匹配行人速度，同时限制机器人最小速度为0.3（看需求，可以不限制）
      else if(predict_person_distance < max_distance_){
        if ((predict_person_vel <= min_x_ && predict_person_distance > (max_distance_ + min_distance_)/2 && fabs(traj.velocity.x - min_x_ - 0.2) < 0.03) ||
            (predict_person_vel <= min_x_ && predict_person_distance <= (max_distance_ + min_distance_)/2 && fabs(traj.velocity.x - min_x_) < 0.03) ||  
            (predict_person_vel > min_x_ && predict_person_distance > max_distance_ - 0.2 && fabs(traj.velocity.x - predict_person_vel - 0.1) < 0.03) ||
            (predict_person_vel > min_x_ && predict_person_distance < min_distance_ + 0.2 && fabs(traj.velocity.x - predict_person_vel + 0.1) < 0.03) ||
            (predict_person_vel > min_x_ && predict_person_distance >= min_distance_ + 0.2 && predict_person_distance <= max_distance_ - 0.2 && fabs(traj.velocity.x - predict_person_vel) < 0.03)){  
            score = 0;  
        }
      }
      else{
    ...
    // 带人模式下
      ...
      score = (traj.velocity.x <= nav_x_) ? 0 : 80; 
    }
