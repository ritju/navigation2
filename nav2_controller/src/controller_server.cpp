  ...
  prune_dist_behind_robot_(1.5)
{
  ...
  // local_costmap_logger.set_level(rclcpp::Logger::Level::Info);
}
...
ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  prune_path_pub_ = node->create_publisher<nav_msgs::msg::Path>("controller_cut_path", 1);
...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  try {
    ...
    if (failure_tolerance_ > 0 || failure_tolerance_ == -1.0) {
      ...
      RCLCPP_WARN(this->get_logger(), e.what());
     throw nav2_core::PlannerException("Run time error !");
  }
...
void ControllerServer::updateGlobalPath()
{
  if (action_server_->is_preempt_requested()) {
    auto now = rclcpp::Clock();
    RCLCPP_WARN_STREAM_THROTTLE(get_logger(), now, 5000, "Passing new path to controller.");
